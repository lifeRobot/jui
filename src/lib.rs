...
pub fn compile(path: impl AsRef<Path>) -> Result<(), CompileError> {
    ...
    not_exists_exec!(return copy_file_lock_version(jui_path,path));
    let file_version = read_to_str_fail_exec!(return copy_file_lock_version(jui_path, path));
    ne_version_exec!(file_version,return copy_file_lock_version(jui_path, path));
...
fn copy_file_lock_version(jui_path: PathBuf, path: impl AsRef<Path>) -> Result<(), CompileError> {
